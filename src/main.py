...
def train(
    model: Model, loader: DataLoaderIAM, line_mode: bool, early_stopping: int = 25
) -> None:
    """Trains NN."""
    ...
    preprocessor = Preprocessor(
        get_img_size(line_mode), data_augmentation=True, line_mode=line_mode
    )
    ...
    while True:
        ...
        if char_error_rate < best_char_error_rate:
            ...
            print(
                f"Character error rate not improved, best so far: {char_error_rate * 100.0}%"
            )
        ...
        if no_improvement_since >= early_stopping:
            print(
                f"No more improvement since {early_stopping} epochs. Training stopped."
            )
            break


def validate(
    model: Model, loader: DataLoaderIAM, line_mode: bool
) -> Tuple[float, float]:
    """Validates NN."""
    print("Validate NN")
...
def main():
    ...
    parser.add_argument(
        "--line_mode",
        help="Train to read text lines instead of single words.",
        action="store_true",
    )
    parser.add_argument(
        "--img_file",
    ...
    parser.add_argument(
        "--early_stopping", help="Early stopping epochs.", type=int, default=25
    )
    parser.add_argument(
    ...
    if args.mode in ["train", "validate"]:
        ...
        open(FilePaths.fn_corpus, "w").write(
            " ".join(loader.train_words + loader.validation_words)
        )
    ...
    # infer text on test image
        ...
        model = Model(
            list(open(FilePaths.fn_char_list).read()),
            decoder_type,
            must_restore=True,
            dump=args.dump,
        )
